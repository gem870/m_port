              ...
              {projects.map((project) => (
                <div key={project._id} onClick={() => setSelectedProject(selectedProject === project._id ? null : project._id)} className="cursor-pointer py-2 group">
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={selectedProject === project._id ? { opacity: 1, height: "auto" } : { opacity: 0, height: 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} className="overflow-hidden">
                    ...
                    <motion.button style={{ border: `2px solid`, borderColor: borderColor, boxShadow: "0px 4px 10px rgba(255, 255, 255, 0.3)" }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex w-fit items-center p-1 gap-2 rounded-md my-4 ml-2 text-[#22ac99] transition-all">
                      <Link href={project.url} >Review</Link>
                    </motion.button>
                  </motion.div>
                </div>
              ))}
            ...
            {selectedProject !== null && (
              <motion.div key={selectedProject} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: "easeOut" }} className="flex h-[40%]">
                {(() => {
                  ...
                  return selectedMedia && isVideo(selectedMedia) ? (
                    <video src={getMediaUrl(selectedMedia)} controls className="rounded-md shadow-lg transition-opacity duration-500 w-full h-auto " />
                  ) : (
                    <div>
                    <img
