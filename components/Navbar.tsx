  ...
  return (
    <>
      ...
      <div className="z-50 fixed w-full font-bold bg-opacity-40 backdrop-blur-3xl bg-black ">
        <motion.section
        style={{ backgroundImage, radius: 10 }} className="hidden md:flex absolute top-4 right-8 rounded-md ">
          <ul className="flex flex-row p-2 space-x-6 border-2 border-gray-600 rounded-md bg-black/50 px-6 py-2">
            {navLinks.map((link, index) => (
              <li key={index}>
                <Link
                  href={link.path}
                  className="transform hover:text-white/60 transition-all text-gray-100 duration-300 ease-in-out"
                >
                  {link.title}
                </Link>
              </li>
      ...
      <div className="z-50 fixed top-5 left-5 md:hidden">
        <div
          onClick={toggleNav}
          className="border rounded text-gray-300 border-gray-300 p-2"
        >
        ...
        style={{ backgroundImage, radius: 10 }}
          className={`fixed top-0 right-0 w-2/3 h-full bg-black/90 transform  transition-transform duration-300 ${
        ...
        >
               {/* Profile Container */}
                  <div className="flex flex-col items-center border-b border-gray-600 p-3 rounded-full shadow-lg w-[100%] max-w-md">
                    ...
                    <p className=" text-center text-gray-300 text-sm">
                      Passionate developer. Loves building innovative solutions. 
                    </p>
          ...
          <ul className="flex flex-col items-center mt-6 space-y-4 h-full">
            {navLinks.map((link, index) => (
              <li key={index}>
                <Link href={link.path} onClick={closeNav} className=" text-white">
                  {link.title}
                </Link>
              </li>
            ))}
